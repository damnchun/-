...
#定义轮廓系数函数，方便其他文件调用
def chioce_best (features_numpy,min_number=2,max_number=10,
                 type = "mini",min_esp = 0.1,max_esp =0.2 ):
    values = []
    #minibach聚类
    if  type == "mini":
        for cluster_number in range(min_number,max_number):
            minibach_program = MiniBatchKMeans(n_clusters=cluster_number,
                                               batch_size=5000)
    ...
    if type == "pu":
        for cluster_number in range(min_number,max_number):
            ...
            value = silhouette_score(X=features_numpy,
                                     labels=clustering_result_result)
            values.append(value)
        #可视化输出
        plt.plot(np.arange(min_number,max_number),values,c="b")
        plt.xlabel("group_number")
    ...
    if type == "db":
        for sample_numbers  in range(min_number,max_number):
            for epsnumber in range (int(10*min_esp),int(10*max_esp)):#循环双参数
                epsn = epsnumber/10
                ...
                if len(np.unique(clustering_result)) < 2:
                    print("Error: Less than two clusters after "
                          "filtering noise. Cannot compute silhouette score.")
                    value = np.nan
                else:
                    # 计算轮廓系数
